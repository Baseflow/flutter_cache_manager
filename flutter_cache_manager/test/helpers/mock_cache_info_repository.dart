...
class MockCacheInfoRepository extends MockCacheInfoRepositoryBase {
  ...
  factory MockCacheInfoRepository() {
    ...
    when(
      provider.insert(any, setTouchedToNow: anyNamed('setTouchedToNow')),
    ...
    when(
      provider.update(any, setTouchedToNow: anyNamed('setTouchedToNow')),
    ).thenAnswer((realInvocation) => Future.value(0));
    when(provider.updateOrInsert(any)).thenAnswer(
      (realInvocation) async =>
          Future.value(realInvocation.positionalArguments.first),
    );
    when(
      provider.getObjectsOverCapacity(any),
    ).thenAnswer((realInvocation) async => Future.value([]));
    when(
      provider.getOldObjects(any),
    ).thenAnswer((realInvocation) async => Future.value([]));
    return provider;
  }
}
